...
## simulate a dataset. There are 1500 patients. The outcome is binary, and there are 5 covariates (age, creatine, diabetes, BMI, and smoking status)
set.seed(123)
data <- tibble(
  patient_id = 1:1500,
  age = rnorm(1500, mean = 50, sd = 10),
  creatine = rnorm(1500, mean = 1.0, sd = 0.2),
...
data <- data %>%
  mutate(risk = age + creatine + diabetes * 2 + BMI - ifelse(smoking_status == "smoker", 1, 0),
...
```{r}
data <- data %>% 
  select(patient_id, age, creatine, diabetes, BMI, smoking_status, outcome)

data
```

### Define inclusion criteria and filter the dataset accordingly
Filter the dataset to include only patients who meet the inclusion criteria. For this example, let's say we want to include patients aged 30-70, with a creatine level between 0.5 and 1.5.

Track of the patients you exclude for your STROBE diagram
```{r}
# Define inclusion criteria
data <- data %>%
  mutate(inclusion = ifelse(age >= 30 & age <= 70 & creatine >= 0.5 & creatine <= 1.5, TRUE, FALSE)) 
...
excl_cr_low <- data %>% filter(creatine < 0.5)
n_excl_cr_low <- nrow(excl_cr_low)
data <- data %>% filter(creatine >= 0.5)

# Exclusion: Creatinine > 1.5
excl_cr_high <- data %>% filter(creatine > 1.5)
n_excl_cr_high <- nrow(excl_cr_high)
data <- data %>% filter(creatine <= 1.5)
...
table_one <- CreateTableOne(vars = c("age", "creatine", "diabetes", "BMI", "smoking_status", "outcome"),
                             data = data,
